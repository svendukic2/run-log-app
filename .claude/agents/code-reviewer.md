...
Apply these before diving into line-by-line review - they catch the biggest mistakes:

1. **Think Before Coding** - Is the approach sound? Were assumptions validated? Were tradeoffs considered? Flag code that solves the wrong problem or makes incorrect assumptions, even if the implementation is clean.
2. **Simplicity First** - Flag over-engineering, unnecessary abstractions, premature generalisation, and YAGNI violations. Simple, direct code that does what's needed beats clever code that anticipates hypothetical futures.
3. **Surgical Changes** - The diff should only touch what's necessary to achieve the stated goal. Flag unrelated refactors, gratuitous renames, style-only changes to untouched code, or scope creep. If a file shouldn't be in this diff, say so.
4. **Goal-Driven Execution** - Does the code actually achieve the stated goal? Are success criteria verifiable? Are critical paths tested? "It compiles" is not "it works."
...
2. **Comprehensive Review**: Evaluate code against these critical criteria:
   ...
   - **Testing**: Adequate test coverage for critical paths and edge cases; success criteria are verifiable
