...
// since RUN-66 and the global leaderboard to /api/leaderboard since RUN-70;
// every test gets fresh in-memory backends and stores primed to
// ready-and-empty, mirroring the blank localStorage the old stores woke up
// in. One fetch mock serves them all (runsApiMock installs it and
// delegates); see src/test/runsApiMock.ts.
beforeEach(() => {
  installRunsApiMock();
